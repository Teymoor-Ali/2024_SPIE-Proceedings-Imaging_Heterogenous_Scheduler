\definecolor{Silver}{rgb}{0.752,0.752,0.752}
\begin{table}
\centering
\caption{Results Table of Vision Pipelines on a Heterogeneous Platform. Total Runtime includes device and model initialisation and Total Energy includes idle processors.}
\label{tab:Individualsummary}
\resizebox{\linewidth}{!}{%
\begin{tblr}{
  cells = {c},
  row{1} = {Silver},
  row{2} = {Silver},
  cell{1}{1} = {r=2}{},
  cell{1}{2} = {c=3}{},
  cell{1}{5} = {r=2}{},
  cell{1}{6} = {r=2}{},
  cell{1}{7} = {r=2}{},
  cell{1}{8} = {r=2}{},
  cell{1}{9} = {r=2}{},
  cell{1}{10} = {r=2}{},
  vlines,
  hline{1,3-6} = {-}{},
  hline{2} = {2-4}{},
}
\textbf{Pipeline} & {\textbf{Accelerator for}\\\textbf{ Partitioned Operation}} &  &  & {\textbf{~ Kernel}\\\textbf{Runtime (ms)}} & {\textbf{Kernel Energy}\\\textbf{(Joules)}} & {\textbf{Kernel}\\\textbf{Frames per }\\\textbf{Second} (\textbf{FPS})} & {\textbf{Total }\\\textbf{Runtime}\\\textbf{(ms)}} & {\textbf{Total }\\\textbf{Energy}\\\textbf{(Joules)}\\\textbf{}} & {\textbf{Accuracy}\\\textbf{(\%)}}\\
 & CPU & GPU & FPGA &  &  &  &  &  & \\
{Histogram \\Equalisation \\+ ResNet-50} & x & Resnet-50 & Hist. & 7.11 & 0.27 & 140.65 & 253.61 & 16.75 & 87\\
{Gaussian Filter\\+ MobilnetV2} & x & MobilnetV2 & Gaus. & 5.22 & 0.17 & 191.57 & 68.17 & 4.85 & 82\\
{RGB2Gray\\+ InceptionV3} & x & InceptionV3 & R2G. & 8.44 & 0.35 & 118.48 & 189.22 & 13.24 & 84
\end{tblr}
}
\end{table}